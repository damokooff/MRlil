...
// ===== AJOUT : Fonction pour générer l'URL de l'image automatiquement =====
function generateWelcomeImageUrl(member, customParams = {}) {
    const username = encodeURIComponent(member.user.username);
    const discriminator = member.user.discriminator || '0000';
    ...
    const defaultParams = {
        ...
        discriminatorColor: 'bdc3c7',
        cacheImage: 'true'
    };
    
    const params = { ...defaultParams, ...customParams };
    const baseUrl = 'https://api.discorddevtools.xyz/welcome-image-generator/generate.png';
    ...
    try {
        ...
        if (welcomeConfig?.enabled && welcomeChannelId) {
            ...
            if (channel?.isTextBased?.()) {
                ...
                // ===== MODIFICATION : Génération auto de l'image si pas d'image configurée =====
                let autoGeneratedImageUrl = null;
                if (!welcomeConfig.welcomeImage && !welcomeConfig.welcomeEmbed?.image?.url) {
                    autoGeneratedImageUrl = generateWelcomeImageUrl(member);
                }
                // ===== FIN MODIFICATION =====

                if (!canEmbed) {
                    // ===== MODIFICATION : Ajout de l'image auto si disponible =====
                    if (autoGeneratedImageUrl) {
                        await channel.send({
                            content: messageContent || welcomeMessage,
                            files: [autoGeneratedImageUrl]
                        });
                    } else {
                        await channel.send({
                            content: messageContent || welcomeMessage
                    ...
                    // ===== AJOUT : Image auto-générée si aucune image configurée =====
                    else if (autoGeneratedImageUrl) {
                        embed.setImage(autoGeneratedImageUrl);
                    }
                    // ===== FIN AJOUT =====
